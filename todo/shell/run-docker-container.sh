...
if which docker &> /dev/null; then
    ...
    echo "${TAG}" > "${DOCKER_TMP_DIR}/${CONTAINER_NAME}.version"
    ...
    if test -z "${CONTAINER_ID}"; then
        if test -f "${CONTAINER_DIRECTORY}/${RUNSCRIPT_FILENAME}"; then
            echo -e "\e[93mRun container '${CONTAINER_NAME}'\e[0m"
            source "${CONTAINER_DIRECTORY}/${RUNSCRIPT_FILENAME}"
        fi
