...
CONTAINERS_IDS_EXITED="$(sudo docker ps --all --no-trunc | grep "Exit " | grep -v "Exit 0" | awk '{print $1}')"
if test -n "${CONTAINERS_IDS_EXITED}"; then
    for CONTAINER_ID_EXITED in ${CONTAINERS_IDS_EXITED}; do
        echo -e "\e[93mRemove container \"${CONTAINER_ID_EXITED}\" (exited, not with exit code 0)\e[0m"
        sudo docker rm -f "${CONTAINER_ID_EXITED}"
    done
fi

# Remove exited containers without a tag
CONTAINERS_IDS_UNTAGGED="$(sudo docker ps --all --no-trunc | grep "Exit 0" | awk '{print $1,$2}' | grep -v ":" | awk '{print $1}')"
if test -n "${CONTAINERS_IDS_UNTAGGED}"; then
    for CONTAINER_ID_UNTAGGED in ${CONTAINERS_IDS_UNTAGGED}; do
        echo -e "\e[93mRemove container \"${CONTAINER_ID_UNTAGGED}\" (exited, without a tag)\e[0m"
