...
run_container_command () {
    local CONTAINER_NAME CONTAINER_ENTRYPOINT="/bin/bash" COMMAND FLYING_ARGUMENT TIME_BEGIN=$(date +%s)
        ...
        case "${PARAMETER}" in
            ...
            *)
                if test -z "${CONTAINER_NAME}"; then
                    CONTAINER_NAME=${1}
                elif test -z "${FLYING_ARGUMENT}"; then
                    FLYING_ARGUMENT=${1}
    ...
    if test -n "${FLYING_ARGUMENT}"; then
        if test -z ${COMMAND}; then
            COMMAND="${FLYING_ARGUMENT}"
        else
            CONTAINER_ENTRYPOINT="${FLYING_ARGUMENT}"
        fi
    fi
    ...
    docker-compose run --rm --entrypoint "${CONTAINER_ENTRYPOINT}" "${CONTAINER_NAME}" -c "${COMMAND}"
}